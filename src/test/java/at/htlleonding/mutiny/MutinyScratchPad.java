...
public class MutinyScratchPad {
    ...
    public void uniFromItem_addEventReactions_retrieveItem()
    {
        ...
        var result = u3.subscribe().with(item -> {booleans[2] = true; integerResult = item;});
    ...
    {
        Uni<Integer> u =
                Uni.createFrom().item(() -> { throw new IllegalArgumentException("Boom"); } );

        var u2 = u.onFailure().recoverWithItem(f -> {hasFailed = true; return -2;});
    ...
    public void uniFromProducer_executeOnAnotherThread_delayed_retrieveItem()
    {
        ...
        var u3 = u2
                .onItem().invoke(()-> {booleans[0] = true; printWithThreadId(".onItem()");})
                .onSubscription().invoke(() -> { booleans[1] = true; printWithThreadId(".onSubscription()"); });

        //use it
        var startAt = System.currentTimeMillis();
        u3.subscribe().with(item -> {booleans[2] = true; integerResult = item; printWithThreadId(".subscribe()"); });
        var finishAt = System.currentTimeMillis();
